"""
Changes needed to accomodate VLM:
- Need to use AutoProcessor instead of AutoTokenizer to handle multi-modality
- Need to change make prompt function to generate dummy image as opposed to dummy text
- remove calls to vocab size
- adjust the model invocations to reflect VLM structure
"""
...
class ElanaProfiler:
    ...
    def __init__(self, args, dtype=torch.bfloat16, device_map="auto"):
        ...
        self.model, self.processor = self._hf_build_model_and_tokenizer()
        self.model.eval()
    ...
    def _make_input(self, batch_size, prompt_len):
        # Generate a dummy image (224x224 works across VLMs; models resize internally)
        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "hello " * prompt_len},
                ],
            }
        ]
    ...
    def profile_size(self, model, batch_size=1, prompt_len=1024, use_GiB=False):
        ...
        inputs = self._make_input(batch_size, prompt_len)
        ...
        if not hasattr(outputs, "past_key_values") or outputs.past_key_values is None:
            raise ValueError("past_key_values is missing in model output")
    ...
    def profile_tpot(self, batch_size=1, prompt_len=1024,
        ...
        if not hasattr(outputs, "past_key_values"):
            raise ValueError("past_key_values missing in prefill output")
        ...
        if cache_graph:
            ...
            static_input_token = next_token.clone()
            with torch.no_grad():
                graph = torch.cuda.CUDAGraph()
                with torch.cuda.graph(graph):
                    static_out = self.model(
                        input_ids=static_input_token,
                        past_key_values=past_key_values,
                        use_cache=True,
                        output_hidden_states=False,
                        output_attentions=False,
            ...
            def generate(new_input_token, new_past_key_values):
                static_input_token.copy_(new_input_token)
        ...
        def _run_once():
            ...
            nonlocal current_token, current_past
            out = generate(current_token, current_past)
            current_past = out.past_key_values
            current_token = out.logits[:, -1, :].argmax(dim=-1).unsqueeze(1)
    ...
    def profile_ttlt(self, batch_size=1, prompt_len=1024, gen_len=128,
        ...
        def run_once(batch_size, prompt_len, gen_len):
            ...
            for _ in range(gen_len - 1):  # one token already generated
                outputs = generate(current_token, current_past)
